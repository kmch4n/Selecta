...
> 誰かに問題集を渡すときは、リンクを送るだけで済みます。さらに `&shuffle=on`・
> `&qshuffle=on`（値は `on`/`off`）を足せば、選択肢・問題順のシャッフルをリンク側から
> 指定できます。指定はその回だけの上書きで、相手の保存設定は変えません。
...
- **問題順シャッフル** — オン/オフできる設定。出題順をランダムにして、並びの記憶に頼らせない。

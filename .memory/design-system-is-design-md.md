...
## 現在のテーマ: modern-minimal / custom-quiet（ライト専用）

2026-07-24 に、Hum（暖色クリーム・pear 黄・playful・丸ゴシック・light/dark）から
**静か・ミニマル（近白 `#fbfcfd` / 濃紺 `#1a2432` / 藍一色 `#2b4570` / ライト専用）**へ全面転換した。
...
- **ライト専用。** `prefers-color-scheme: dark` のブロックを足さない。`:root` に
  `color-scheme: light` があり、OS がダークでも UA 部品ごとライトに保つ。
